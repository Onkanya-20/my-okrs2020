...
export const minLength = minRequired =>
  compose(minLengthError(minRequired), minLengthValidate(minRequired));

export const maxLength = maxRequired =>
  compose(maxLengthError(maxRequired), maxLengthValidate(maxRequired));

export const email = compose(emailError, emailValidate);
...
export const wordCharacter = fieldName =>
  compose(wordCharacterError(fieldName), wordCharacterValidate);

export const isSame = comparedField => (value, values) =>
  compose(isSameError, isSameValidate)(value, values[comparedField]);
