...
describe('provideVariant', () => {
  ...
  describe('should return empty string if provided variant prop is not match', () => {
    ...
    const renderTextAndGetColor = type => {
      ...
      const color = window.getComputedStyle(getByText('Text')).color;
      return color;
    };

    test('primary color', () => {
      const primaryColor = renderTextAndGetColor('primary');

      expect(primaryColor).toBe('blue');
    });

    test('danger color', () => {
      const dangerColor = renderTextAndGetColor('danger');

      expect(dangerColor).toBe('red');
    });

    test('secondary color', () => {
      const secondaryColor = renderTextAndGetColor('secondary');

      expect(secondaryColor).toBe('green');
    });

    test('default color', () => {
      const defaultColor = renderTextAndGetColor();
